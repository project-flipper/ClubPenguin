...
export default class Town extends Phaser.Scene implements Room {
    ...
    preload(): void {
        ...
        this.load.pack("town2013-pack", "assets/world/rooms/2013/town/town2013-pack.json");
    }

    editorCreate(): void {

        // sky
        const sky = this.add.image(-22.5, -22.5, "town2013", "town/sky");
        sky.setOrigin(0, 0);

        // clouds
        const clouds = this.add.image(-22.5, 53.6625, "town2013", "town/clouds");
        clouds.setOrigin(0, 0);

        // mountains
        const mountains = this.add.image(111.0375, 85.6125, "town2013", "town/mountains");
        mountains.setOrigin(0, 0);

        // danceDoorArea
        const danceDoorArea = this.add.image(901.35, 345.03745, "town2013", "town/danceinside");
        danceDoorArea.setOrigin(0, 0);

        // danceDoor
        const danceDoor = this.add.image(947, 403, "town2013", "town/dancedoor");

        // base
        const base = this.add.image(-119.25, 49.6125, "town2013", "town/base");
        base.setOrigin(0, 0);

        // town_newsboard
        const town_newsboard = this.add.image(226.2375, 773.775, "town2013", "town/newsboard");
        town_newsboard.setOrigin(0.4937, 0.32853);

        // town_sidetrees
        const town_sidetrees = this.add.image(1695.0375, 703.35, "town2013", "town/sidetrees");
        town_sidetrees.setOrigin(0.49165, 0.49622);

        // town_bottom
        const town_bottom = this.add.image(864.1125, 1112.1625, "town2013", "town/bottom");
        town_bottom.setOrigin(0.5, 1);

        // shopDoor
        const shopDoor = this.add.image(1208.7, 335.1375, "town2013", "town/shopdoor0001");
        shopDoor.setOrigin(0, 0);

        // coffeeDoor
        const coffeeDoor = this.add.image(422.325, 396.675, "town2013", "town/coffeedoor0001");
        ...
        // spotlightRight
        const spotlightRight = this.add.image(1069.0875, 250.3125, "town2013", "town/spotlightright");
        spotlightRight.setOrigin(0.5, 1);

        // spotlightContentLeft
        const spotlightContentLeft = this.add.image(618.75, -22.5, "town2013", "town/spotlightcontent");
        spotlightContentLeft.setOrigin(0, 0);

        // spotlightContentRight
        const spotlightContentRight = this.add.image(956.25, -22.5, "town2013", "town/spotlightcontent");
        spotlightContentRight.setOrigin(0, 0);

        // spotlightMaskLeft
        const spotlightMaskLeft = this.add.image(824.7375, 242.8875, "town2013", "town/spotlightmaskleft");
        spotlightMaskLeft.setOrigin(0.62469697, 1.13786561);
        spotlightMaskLeft.visible = false;

        // spotlightMaskRight
        const spotlightMaskRight = this.add.image(1069.0875, 250.3125, "town2013", "town/spotlightmaskright");
        spotlightMaskRight.setOrigin(0.47330189, 1.13580769);
        spotlightMaskRight.visible = false;

        // town_shopfronts
        const town_shopfronts = this.add.image(376.9875, 235.8, "town2013", "town/shopfronts");
        town_shopfronts.setOrigin(0, 0);

        // storenames
        const storenames = this.add.image(222.4, 177.075, "town2013", "town/storenames0001");
        storenames.setOrigin(0, 0);

        // town_shopsign
        const town_shopsign = this.add.image(1159.875, 140.7375, "town2013", "town/shopsign");
        town_shopsign.setOrigin(0, 0);

        // town_cord
        const town_cord = this.add.image(1062.45, 502.65, "town2013", "town/cord");
        town_cord.setOrigin(0.47154, 0.54857);

        // town_leftchair
        const town_leftchair = this.add.image(608.9625, 510.525, "town2013", "town/leftchair");
        town_leftchair.setOrigin(0.47593, 0.35102);

        // town_rightchair
        const town_rightchair = this.add.image(783.225, 500.85, "town2013", "town/rightchair");
        town_rightchair.setOrigin(0.46987, 0.37142);

        // town_table
        const town_table = this.add.image(697.725, 556.0875, "town2013", "town/table");
        town_table.setOrigin(0.48025, 0.68182);

        // town_benchside
        const town_benchside = this.add.image(289.35, 612, "town2013", "town/benchside");
        town_benchside.setOrigin(0.47066, 0.70566);

        // town_clothes
        const town_clothes = this.add.image(1347.75, 542.925, "town2013", "town/clothes");
        town_clothes.setOrigin(0.48008, 0.585);

        // town_dancecover
        const town_dancecover = this.add.image(951.975, 496.6875, "town2013", "town/dancecover");
        town_dancecover.setOrigin(0.48726, 0.9);

        // discolights
        const discolights = this.add.sprite(883, 328, "town2013", "town/discolights0001");
        discolights.setOrigin(0, 0);

        // town_dancestars
        const town_dancestars = this.add.image(887.4, 330.75, "town2013", "town/dancestars");
        town_dancestars.setOrigin(0, 0);

        // purplelight
        const purplelight = this.add.image(725.9625, 102.2625, "town2013", "town/purplelight");
        purplelight.setOrigin(0, 0);

        // bluelight
        const bluelight = this.add.image(779.9625, 121.3875, "town2013", "town/bluelight");
        bluelight.setOrigin(0, 0);

        // greenlight
        const greenlight = this.add.image(821.925, 112.1625, "town2013", "town/greenlight");
        greenlight.setOrigin(0, 0);

        // bluelight2
        const bluelight2 = this.add.image(1027.8, 108.1125, "town2013", "town/altbluelight");
        bluelight2.setOrigin(0, 0);

        // redlight
        const redlight = this.add.image(1088.6625, 103.5, "town2013", "town/redlight");
        redlight.setOrigin(0, 0);

        // greenlight2
        const greenlight2 = this.add.image(1115.8875, 125.4375, "town2013", "town/altgreenlight");
        greenlight2.setOrigin(0, 0);

        // block
        const block = this.add.image(0, 0, "town2013", "town/block");
        block.setOrigin(0, 0);
        block.visible = false;
        ...
        // town_dance_trigger
        const town_dance_trigger = this.add.image(949.5, 478.35, "town2013", "town/dance_trigger");
        town_dance_trigger.visible = false;

        // town_dock_trigger
        const town_dock_trigger = this.add.image(87.8625, 647.1, "town2013", "town/dock_trigger");
        town_dock_trigger.visible = false;

        // town_forts_trigger
        const town_forts_trigger = this.add.image(1586.25, 640.35, "town2013", "town/forts_trigger");
        town_forts_trigger.visible = false;

        // town_shop_trigger
        const town_shop_trigger = this.add.image(1230.75, 503.1, "town2013", "town/shop_trigger");
        ...
        shopDoorButtonComponent.upTexture = {"key":"town2013","frame":"town/shopdoor0001"};
        shopDoorButtonComponent.overTexture = {"key":"town2013","frame":"town/shopdoor0002"};
        ...
        coffeeDoorButtonComponent.upTexture = {"key":"town2013","frame":"town/coffeedoor0001"};
        coffeeDoorButtonComponent.overTexture = {"key":"town2013","frame":"town/coffeedoor0002"};
    ...
    create(data: any): void {
        ...
        this.coffeeDoor.on('over', () => this.sound.play('town2013-coffeeopen'));
        this.coffeeDoor.on('out', () => this.sound.play('town2013-coffeeclose'));
        this.coffeeDoor.on('release', () => this.world.move(483.75, 528.75));

        this.shopDoor.on('over', () => this.sound.play('town2013-shopopen'));
        this.shopDoor.on('out', () => this.sound.play('town2013-shopclose'));
    ...
    unload(engine: Engine): void {
        ...
        engine.app.unloadAssetPack('town2013-pack');
    }
