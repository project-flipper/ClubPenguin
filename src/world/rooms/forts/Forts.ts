...
export default class Forts extends Phaser.Scene {
    ...
    preload(): void {
        ...
        this.load.pack("forts-pack", "assets/world/rooms/forts/forts-pack.json");
    }

    editorCreate(): void {

        // forts_sky
        const forts_sky = this.add.image(-22.5, -22.5, "forts", "forts/sky");
        forts_sky.setOrigin(0, 0);

        // forts_base
        const forts_base = this.add.image(-157.3875, 103.3875, "forts", "forts/base");
        forts_base.setOrigin(0, 0);

        // forts_snowman
        const forts_snowman = this.add.image(1549.0125, 864.9, "forts", "forts/snowman");
        forts_snowman.setOrigin(0.48875598086124405, 0.8522222222222222);

        // forts_orangecastletower
        const forts_orangecastletower = this.add.image(974.475, 745.65, "forts", "forts/orangecastletower");
        forts_orangecastletower.setOrigin(0.5005084745762712, 0.4032857142857143);

        // forts_orangecastlebase
        const forts_orangecastlebase = this.add.image(542.025, 743.2875, "forts", "forts/orangecastlebase");
        forts_orangecastlebase.setOrigin(0.49957346, 0.56315175);

        // orangeflag
        const orangeflag = this.add.sprite(437.5125, 477.225, "forts", "forts/orangeflag0001");
        orangeflag.setOrigin(0, 0);

        // forts_orangecastleside
        const forts_orangecastleside = this.add.image(861.75, 789.0749999999999, "forts", "forts/orangecastleside");
        forts_orangecastleside.setOrigin(0.5, 0.542007434944238);

        // forts_orangecastlepillar
        const forts_orangecastlepillar = this.add.image(619.2, 855.5625, "forts", "forts/orangecastlepillar");
        forts_orangecastlepillar.setOrigin(0.498974358974359, 0.63);

        // forts_bluecastlepillar
        const forts_bluecastlepillar = this.add.image(1195.65, 494.2125, "forts", "forts/bluecastlepillar");
        forts_bluecastlepillar.setOrigin(0.49734939759036145, 0.5923469387755101);

        // forts_bluecastleotherwall
        const forts_bluecastleotherwall = this.add.image(982.6875, 461.5875, "forts", "forts/bluecastleotherwall");
        forts_bluecastleotherwall.setOrigin(0.4986792452830189, 0.7568181818181817);

        // forts_bluecastletower
        const forts_bluecastletower = this.add.image(853.3125, 493.2, "forts", "forts/bluecastletower");
        forts_bluecastletower.setOrigin(0.4978723404255319, 0.3824);

        // forts_bluecastlebase
        const forts_bluecastlebase = this.add.image(946.8, 558.5625, "forts", "forts/bluecastlebase");
        forts_bluecastlebase.setOrigin(0.500662251655629, 0.7421812080536913);

        // forts_bluecastleside
        const forts_bluecastleside = this.add.image(1349.1, 588.9375, "forts", "forts/bluecastleside");
        forts_bluecastleside.setOrigin(0.5008064516129033, 0.34600000000000003);

        // forts_bluecastlewall
        const forts_bluecastlewall = this.add.image(1202.4, 626.4, "forts", "forts/bluecastlewall");
        forts_bluecastlewall.setOrigin(0.49943966, 0.69815789);

        // blueflag
        const blueflag = this.add.sprite(838.575, 337.1625, "forts", "forts/blueflag0001");
        blueflag.setOrigin(0, 0);

        // cog
        const cog = this.add.sprite(1444.6125, 124.0875, "forts", "forts/cog0001");
        cog.setOrigin(0, 0);

        // forts_clockbase
        const forts_clockbase = this.add.image(1078.0875, -13.95, "forts", "forts/clockbase");
        forts_clockbase.setOrigin(0, 0);

        // target
        const target = this.add.sprite(1412.8875, 139.1625, "forts", "forts/target0001");
        target.setOrigin(0, 0);

        // clockwork
        const clockwork = this.add.sprite(1108.575, 108.1125, "forts", "forts/clockwork0001");
        clockwork.setOrigin(0, 0);

        // clocktext
        const clocktext = this.add.image(1145.8125, 62.55, "forts", "forts/clocktext0001");
        ...
        // snowball_target
        const snowball_target = this.add.image(1469, 175, "forts", "forts/target_area");
        snowball_target.visible = false;

        // block
        const block = this.add.image(0, 0, "forts", "forts/block");
        block.setOrigin(0, 0);
        block.visible = false;

        // town_btn
        const town_btn = this.add.image(207.5625, 355.3875, "forts", "forts/town_btn0004");
        town_btn.alpha = 0.01;
        ...
        // plaza_btn
        const plaza_btn = this.add.image(1586.25, 303.75, "forts", "forts/plaza_btn0004");
        ...
        // rink_btn
        const rink_btn = this.add.image(517.5, 263.8125, "forts", "forts/rink_btn0004");
        rink_btn.alpha = 0.01;
        ...
        // forts_plaza_trigger
        const forts_plaza_trigger = this.add.image(1570.275, 454.1625, "forts", "forts/plaza_trigger");
        forts_plaza_trigger.visible = false;

        // forts_rink_trigger
        const forts_rink_trigger = this.add.image(589.5, 382.1625, "forts", "forts/rink_trigger");
        forts_rink_trigger.visible = false;

        // forts_town_trigger
        const forts_town_trigger = this.add.image(159.75, 546.4125, "forts", "forts/town_trigger");
